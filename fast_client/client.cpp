...
fstream FileCopy("E:\\Resource\\Dynamic_Wallpapers\\BigSur\\Big macOS Sur-21114.jpg", ios::out | ios::binary);
...
int main()
{
	...
	// Print time
	start = time(&start);
	ctime_s(timeBuff, sizeof timeBuff, &start);
	...
	ctime_s(timeBuff, sizeof timeBuff, &end);
	cout << "Sent successfully, cost " << end - start << "s" << endl;
